...
class DesktopPet(QMainWindow):
    def __init__(self):
        ...
        self.notice_queue = deque(maxlen=5)  # 提醒队列，防止短时间多个提醒丢失
    ...
    def _create_tray(self):
        ...
        # 暂时收起 / 恢复桌宠
        self.act_hide_tray = QAction("暂时收起桌宠（隐藏窗口）", self)
        self.act_hide_tray.triggered.connect(self._toggle_manual_hide)
        menu.addAction(self.act_hide_tray)
    ...
    def contextMenuEvent(self, event):
        ...
        # 本体右键同样提供暂时收起入口
        self.act_hide_ctx = QAction("暂时收起桌宠（隐藏窗口）", self)
        self.act_hide_ctx.triggered.connect(self._toggle_manual_hide)
        menu.addAction(self.act_hide_ctx)
    ...
    # ---------- 显隐控制（暂时收起 / 恢复） ----------
    def _update_hide_actions_text(self):
        """根据当前可见状态更新托盘和右键菜单文案。"""
        text_show = "暂时收起桌宠（隐藏窗口）"
        text_hide = "恢复桌宠到桌面"
        if self.isVisible():
            new_text = text_show
        else:
            new_text = text_hide
        for act in (getattr(self, "act_hide_tray", None), getattr(self, "act_hide_ctx", None)):
            try:
                if act is not None:
    ...
    def _toggle_manual_hide(self):
        """手动切换：暂时收起/恢复桌宠，不依赖全屏检测。"""
        if self.isVisible():
            self._hidden_manually = True
            self.hide()
        else:
            self._hidden_manually = False
            self.show()
            self.raise_()
            self.activateWindow()
        self._update_hide_actions_text()
...
# --- reminders ---
    ...
    def _resolve_app(self, exe: str, title: str):
        ...
        if not category:
            # 浏览器默认 browse，其他未识别应用用 browse 作为通用兜底（与 category_pool 一致）
            browser_exes_check = ["chrome.exe", "msedge.exe", "firefox.exe", "brave.exe", "opera.exe", "vivaldi.exe", "iexplore.exe", "safari.exe"]
            category = "browse" if exe_l in browser_exes_check else "browse"
